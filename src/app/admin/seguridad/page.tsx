...
const EVENT_TYPES = [
  { value: '', label: 'Todos' },
  { value: 'LOGIN_SUCCESS', label: '✅ Login OK' },
...
export default function SeguridadPage() {
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      const response = await adminFetch(`/api/admin/security-logs${params}`, {}, accessToken, csrfToken);
  ...
  const getEventIcon = (tipo: string) => {
    if (tipo.includes('SUCCESS')) return <CheckCircle className="w-4 h-4 text-emerald-400" />;
    if (tipo.includes('FAILED')) return <XCircle className="w-4 h-4 text-red-400" />;
    if (tipo.includes('INJECTION') || tipo.includes('XSS')) return <AlertTriangle className="w-4 h-4 text-red-500" />;
    return <Eye className="w-4 h-4 text-gray-400" />;
  };

  const getEventColor = (tipo: string) => {
    if (tipo.includes('SUCCESS')) return 'border-l-emerald-500 bg-emerald-500/5';
    if (tipo.includes('FAILED')) return 'border-l-red-500 bg-red-500/5';
